...
IUSE="abi_mips_n32 audit debug dinit kerberos ldns libedit livecd pam +pie security-key selinux +ssl static test xmss"
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/ssh-keygen
		doins "${FILESDIR}"/dinit/sshd

		insinto /etc/dinit.d/user
		doins "${FILESDIR}"/dinit/ssh-agent
	fi
...
openssh_maybe_restart() {
	...
	if [[ ${ROOT} ]]; then
		...
		eend $?
	elif [[ -S /run/dinitctl ]] && sshd -t > /dev/null 2>&1; then
		local status=$(dinitctl is-started sshd 2>/dev/null)

		if [[ ${status} == "STARTED" || ${status} == "STARTING" ]]; then
			ewarn "The ebuild will now attempt to restart OpenSSH to avoid"
			ewarn "bricking the running instance. See bug #709748."
			ebegin "Attempting to restart openssh via 'dinitctl restart sshd'"
