...
LICENSE="GPL-2"
SLOT="0"
IUSE="+caps +cmdmon debug dinit html libtomcrypt +nettle nss +nts +phc pps +readline +refclock +rtc samba +seccomp +sechash selinux"
...
src_prepare() {
	...
	if use dinit; then
		eapply "${FILESDIR}"/dinit/max_resolve_interval.patch
		eapply "${FILESDIR}"/dinit/no-pidfile-default.patch
	fi
}
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/chrony
		doins "${FILESDIR}"/dinit/chronyd

		exeinto /usr/lib/dinit
		newexe "${FILESDIR}"/dinit/chronyd.script chronyd
	fi
