...
IUSE="audit bluetooth +concheck connection-sharing debug dhclient dhcpcd dinit elogind gnutls gtk-doc +introspection iptables iwd psl libedit +nss nftables +modemmanager ofono ovs policykit +ppp resolvconf selinux syslog systemd teamd test +tools vala +wext +wifi"
RESTRICT="!test? ( test )"
...
multilib_src_install_all() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/NetworkManager
	fi
...
pkg_postinst() {
	...
	if [[ -n "${REPLACING_VERSIONS}" ]]; then
		if use dinit; then
			elog
			elog "The NetworkManager service can be enabled by running:"
			elog "dinitctl enable NetworkManager"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../NetworkManager /etc/dinit.d/boot.d/"
			elog
		fi
	fi
}
