...
IUSE="+anacron +dinit-init +inotify openrc pam selinux systemd"
...
pkg_postinst() {
	...
	if [[ -n "${REPLACING_VERSIONS}" ]] ; then
		...
		if use dinit-init ; then
			elog
			elog "The crond service can be enabled by running:"
			elog "dinitctl enable crond"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../crond /etc/dinit.d/boot.d/"
			elog
		fi
	fi
}
