...
# TODO: USE=daemon
IUSE="debug +dinit +dinit-init doc elogind openrc selinux static-libs systemd test valgrind X"
RESTRICT="!test? ( test )"
...
RDEPEND="
	${COMMON_DEPEND}
	acct-user/messagebus
	dinit-init? ( >=app-shells/bash-5.2_p37-r3 )
...
src_prepare() {
	...
	if use dinit || use dinit-init; then
...
multilib_src_configure() {
	...
	if use dinit || use dinit-init; then
		emesonargs+=( -Dtraditional_activation=true )
	fi
...
pkg_postinst() {
	...
	if [[ -z "${REPLACING_VERSIONS}" ]] && use dinit-init; then
		elog
		elog "The dbus service can be enabled by running:"
		elog "dinitctl enable dbus"
		elog
		elog "In a chroot environment the service can be enabled by running:"
		elog "ln -s ../dbus /etc/dinit.d/boot.d/"
		elog
	fi
}
