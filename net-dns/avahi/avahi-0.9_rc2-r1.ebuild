...
IUSE="autoipd bookmarks +dbus dinit doc gdbm gtk howl-compat +introspection ipv6 mdnsresponder-compat nls python qt5 selinux systemd test"
...
multilib_src_configure() {
	...
	if use dinit; then
		myconf+=( --with-avahi-priv-access-group=network )
	fi
...
multilib_src_install_all() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/avahi-daemon
	fi
...
pkg_postinst() {
	...
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		if use dinit; then
			elog
			elog "The avahi-daemon service can be enabled by running:"
			elog "dinitctl enable avahi-daemon"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../avahi-daemon /etc/dinit.d/boot.d/"
			elog
		fi
	fi
}
