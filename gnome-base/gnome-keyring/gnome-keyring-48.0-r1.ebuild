...
IUSE="caps dinit pam selinux ssh-agent systemd test"
RESTRICT="!test? ( test )"
...
src_install() {
	...
	if use dinit; then
		...
		newins "${FILESDIR}"/dinit/gnome-keyring-daemon.user gnome-keyring-daemon
		newins "${FILESDIR}"/dinit/gnome-keyring-daemon-pre.user gnome-keyring-daemon-pre

		exeinto /usr/lib/dinit/user/pre
		newexe "${FILESDIR}"/dinit/gnome-keyring-daemon.user.script gnome-keyring-daemon
	fi
}
