...
IUSE="corosync dinit static systemd +udev"
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/mdadm
		doins "${FILESDIR}"/dinit/mdraid
	fi
	...
	if ! systemd_is_booted; then
		if [[ -z ${REPLACING_VERSIONS} ]] ; then
			...
			if use dinit; then
				...
				elog "devices, you need to enable the mdraid service."
				elog
				elog "Services can be enabled by running:"
				elog "dinitctl enable mdadm"
				elog "dinitctl enable mdraid"
				elog
				elog "In a chroot environment the services can be enabled by running:"
				elog "ln -s ../mdadm /etc/dinit.d/boot.d/"
				elog "ln -s ../mdraid /etc/dinit.d/boot.d/"
				elog
			else
				elog "If you're not relying on kernel auto-detect of your RAID"
