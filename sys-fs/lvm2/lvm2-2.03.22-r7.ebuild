...
IUSE="+dinit-init lvm openrc readline sanlock selinux static static-libs systemd thin +udev valgrind"
REQUIRED_USE="
	static? ( !systemd !udev )
...
src_install() {
	...
	if use lvm ; then
		if use openrc ; then
			...
			insinto /etc/dinit.d
			doins "${FILESDIR}"/dinit/dmeventd
			doins "${FILESDIR}"/dinit/lvm2
			doins "${FILESDIR}"/dinit/lvm2-monitor
			doins "${FILESDIR}"/dinit/lvmetad
			doins "${FILESDIR}"/dinit/lvmpolld
			doins "${FILESDIR}"/dinit/lvmpolld-pre

			dodir /etc/dinit.d/mount.d
			dosym /etc/dinit.d/lvm2 /etc/dinit.d/mount.d/lvm2
		fi
	...
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		if use lvm ; then
			...
			if use dinit-init ; then
				elog
				elog "Services can be enabled by running:"
				elog "dinitctl enable dmeventd"
				elog "dinitctl enable lvm2-monitor"
				elog "dinitctl enable lvmetad"
				elog "dinitctl enable lvmpolld"
				elog
				elog "In a chroot environment services can be enabled by running:"
				elog "ln -s ../dmeventd /etc/dinit.d/boot.d/"
				elog "ln -s ../lvm2-monitor /etc/dinit.d/boot.d/"
				elog "ln -s ../lvmetad /etc/dinit.d/boot.d/"
				elog "ln -s ../lvmpolld /etc/dinit.d/boot.d/"
				elog
			fi
		else
