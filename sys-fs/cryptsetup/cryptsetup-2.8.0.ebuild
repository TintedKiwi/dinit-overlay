...
IUSE="${CRYPTO_BACKENDS} +argon2 dinit fips nls pwquality passwdqc ssh static static-libs test +udev urandom"
RESTRICT="!test? ( test )"
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/cryptsetup
		doins "${FILESDIR}"/dinit/cryptsetup-early

		exeinto /usr/lib/dinit
		newexe "${FILESDIR}"/dinit/cryptsetup.script cryptsetup
	fi
...
pkg_postinst() {
	...
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		if use dinit; then
			elog
			elog "The cryptsetup service can be enabled by running:"
			elog "dinitctl enable cryptsetup"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../cryptsetup /etc/dinit.d/boot.d/"
			elog
		fi
	fi
