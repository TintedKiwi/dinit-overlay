...
SLOT="0/0.4"
IUSE="${PIPEWIRE_DOCS_USEFLAG} bluetooth elogind dbus dinit doc echo-cancel extra ffmpeg fftw flatpak gstreamer gsettings ieee1394 jack-client jack-sdk liblc3 loudness lv2"
...
src_prepare() {
	...
	if use dinit; then
		eapply "${FILESDIR}"/dinit/readiness.patch
	fi
}
...
multilib_src_install_all() {
	...
	if use dinit; then
		...
		newins "${FILESDIR}"/dinit/pipewire.user pipewire
	fi
	...
	if use sound-server; then
		...
		if use dinit; then
			insinto /etc/dinit.d/user
			newins "${FILESDIR}"/dinit/pipewire-pulse.user pipewire-pulse
		fi
