...
LICENSE="GPL-2"
SLOT="0"
IUSE="+client cpu_flags_x86_aes cpu_flags_x86_ssse3 +dinit +monitor ofono selinux standalone systemd wired"
...
src_prepare() {
	...
	if use dinit; then
		eapply "${FILESDIR}"/dinit/dbus-services.patch

		if use standalone; then
			eapply "${FILESDIR}"/dinit/resolvconf-default.patch
		fi
	else
		sed -e "s:Exec=/bin/false:Exec=${EPREFIX}/usr/libexec/iwd:g" -i src/net.connman.iwd.service || die

		if use wired; then
			sed -e "s:Exec=/bin/false:Exec=${EPREFIX}/usr/libexec/ead:g" -i wired/net.connman.ead.service || die
		fi
	fi
}
...
src_configure() {
	...
	if use systemd; then
		myeconfargs+=( --enable-systemd-service )
	else
		myeconfargs+=( --disable-systemd-service )
	fi
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/iwd

		if use wired; then
			doins "${FILESDIR}"/dinit/ead
		fi
	else
...
pkg_postinst() {
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		if use dinit && use standalone; then
			elog
			elog "Services can be enabled by running:"

			if use wired; then
				elog "dinitctl enable ead"
			fi

			elog "dinitctl enable iwd"
			elog
			elog "In a chroot environment services can be enabled by running:"
			elog "ln -s ../service /etc/dinit.d/boot.d/"
			elog
		fi
	fi
}
