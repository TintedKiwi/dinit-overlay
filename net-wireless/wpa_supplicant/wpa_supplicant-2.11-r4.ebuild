...
SLOT="0"
IUSE="+ap broadcom-sta dbus dinit eap-sim eapol-test +fils gui macsec +mbo +mesh p2p privsep readline selinux smartcard tkip uncommon-eap-types wep wps"
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/wpa_supplicant

		insinto /etc/dinit.d/config
		doins "${FILESDIR}"/dinit/wpa_supplicant.conf
	fi
...
pkg_postinst() {
	...
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		if use dinit; then
			elog
			elog "The wpa_supplicant service can be enabled by running:"
			elog "dinitctl enable wpa_supplicant"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../wpa_supplicant /etc/dinit.d/boot.d/"
			elog
		fi
	fi
}
