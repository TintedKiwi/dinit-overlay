...
IUSE="btpclient cups debug deprecated dinit doc extra-tools experimental man +mesh midi +obex +readline selinux systemd test test-programs +udev"
...
src_prepare() {
	...
	if use dinit; then
		eapply "${FILESDIR}"/dinit/0001-enable-dbus-without-systemd.patch
		eapply "${FILESDIR}"/dinit/0001-src-org.bluez.service-start-dinit-service.patch
		eapply "${FILESDIR}"/dinit/0008-grant-permission-to-bluetooth-group.patch

		if use mesh; then
			eapply "${FILESDIR}"/dinit/0003-allow-mesh-on-non-systemd-system.patch
		fi
	fi
...
multilib_src_install_all() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/bluetoothd
	fi
