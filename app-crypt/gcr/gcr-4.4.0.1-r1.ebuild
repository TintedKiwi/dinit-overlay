...
IUSE="dinit gnutls gtk gtk-doc +introspection systemd test +vala"
REQUIRED_USE="
...
src_install() {
	...
	if use dinit; then
		...
		newins "${FILESDIR}"/dinit/gcr-ssh-agent.user gcr-ssh-agent
		newins "${FILESDIR}"/dinit/gcr-ssh-agent-pre.user gcr-ssh-agent-pre

		exeinto /usr/lib/dinit/user/pre
		newexe "${FILESDIR}"/dinit/gcr-ssh-agent.user.script gcr-ssh-agent
	fi
