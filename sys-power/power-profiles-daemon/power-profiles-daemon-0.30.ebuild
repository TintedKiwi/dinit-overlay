...
IUSE="bash-completion dinit gtk-doc man selinux test zsh-completion"
...
src_prepare() {
	...
	if use dinit; then
		eapply "${FILESDIR}"/dinit/readiness-dbus.patch
	fi
}
...
src_install() {
	...
	if use dinit; then
		insinto /etc/dinit.d
		doins "${FILESDIR}"/dinit/power-profiles-daemon
...
pkg_postinst() {
	if [[ -z "${REPLACING_VERSIONS}" ]] && ! use dinit; then
