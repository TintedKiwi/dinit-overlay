...
IUSE="clickhouse curl dbi debug +dinit doc elasticsearch +gcrypt gnutls imdocker imhttp"
...
RDEPEND="
	...
	dinit? (
		>=app-admin/logrotate-3.22.0[cron]
...
pkg_postinst() {
	...
	if [[ -z "${REPLACING_VERSIONS}" ]]; then
		...
		if use dinit; then
			elog
			elog "The rsyslogd service can be enabled by running:"
			elog "dinitctl enable rsyslogd"
			elog
			elog "In a chroot environment the service can be enabled by running:"
			elog "ln -s ../rsyslogd /etc/dinit.d/boot.d/"
			elog
		fi
	fi
