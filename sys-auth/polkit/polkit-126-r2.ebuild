...
IUSE="+dinit doc examples gtk +introspection kde +pam nls selinux systemd test +turnstile"
REQUIRED_USE="
	dinit? ( pam turnstile )
"
RESTRICT="!test? ( test )"
...
src_prepare() {
	...
	if use dinit; then
		...
		eapply "${FILESDIR}"/dinit/disable-sd-pidfd.patch
		eapply "${FILESDIR}"/dinit/readiness.patch
	fi

	if use turnstile; then
		eapply "${FILESDIR}"/turnstile/turnstile.patch
	fi
}
