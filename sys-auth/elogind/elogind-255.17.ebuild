...
SLOT="0"
IUSE="+acl audit cgroup-hybrid debug +dinit-init doc openrc +pam +policykit rundir selinux test +turnstile"
RESTRICT="!test? ( test )"
...
RDEPEND="${DEPEND}
	...
	turnstile? (
		rundir? ( >=sys-auth/turnstile-0.1.10[-rundir] )
		!rundir? ( >=sys-auth/turnstile-0.1.10[rundir] )
...
"
PDEPEND="
	dinit-init? ( >=sys-apps/dbus-1.16.2[dinit-init] )
	!dinit-init? ( sys-apps/dbus )
...
src_prepare() {
	...
	if use dinit-init; then
		mkdir "${WORKDIR}"/dinit
		cp "${FILESDIR}"/dinit/elogind "${WORKDIR}"/dinit/

		eapply "${FILESDIR}"/dinit/dbus-activation.patch

		if use policykit; then
			sed -i '/^depends-on\s*=\s*dbus$/i depends-ms         = polkitd' "${WORKDIR}/dinit/elogind" || die
		fi
	fi

	if ! use turnstile; then
		sed -i '/^before\s*=\s*login\.target$/a before             = logind' "${WORKDIR}/dinit/elogind" || die
	else
		eapply "${FILESDIR}"/turnstile/linger-path.patch

		if ! use rundir; then
			eapply "${FILESDIR}"/turnstile/no-runtime-dir.patch
		fi
	fi
}
...
src_install() {
	...
	if use openrc; then
		...
		insinto /etc/dinit.d
		doins "${WORKDIR}"/dinit/elogind
		doins "${FILESDIR}"/dinit/elogind-cg

		if ! use turnstile; then
			doins "${FILESDIR}"/dinit/logind
