...
SLOT="0"
IUSE="+acl audit cgroup-hybrid debug dinit doc +pam +policykit selinux test turnstile"
REQUIRED_USE="
	dinit? ( !cgroup-hybrid )
	turnstile? ( dinit )
"
...
DEPEND="
	...
	dinit? ( >=sys-libs/cgroup-utils-0.7.2 )
	pam? ( sys-libs/pam )
...
PDEPEND="
	...
	turnstile? ( >=sys-auth/turnstile-0.1.10 )
	sys-apps/dbus
"
...
pkg_postinst() {
	...
	if use turnstile; then
		ewarn "elogind no longer manages the user runtime directory (XDG_RUNTIME_DIR)"
		ewarn "nor service lingering. Turnstile by default now manages these duties."
		ewarn
	fi
